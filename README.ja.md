...
- `summarize-k1-entry.sh`
  - baseline/side の split から candidate 抽出と hsaco マップを連結し、K1..K4 の入口状態を1表に集約する補助。
- `g4-k1-single-shape-loop.sh`
  - 本命1shape（`512x512x2880`）の K1 入口ループ。`ROCBLAS_TENSILE_LIBPATH` だけを変える one-point A/B で、`RUN_TAG` 命名の canonical 証跡、shape hit、`ttft/total/tok_s` を1回で記録する補助。
- `summarize-k1-single-shape-repeats.sh`
  - `g4_k1_single_shape_loop_<RUN_TAG>*.tsv` を集約し、laneごとの平均/最小/最大と安定性フラグを出す repeat 要約補助。
- `commit-no-raw.sh`
