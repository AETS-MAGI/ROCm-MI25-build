...
- [ ] MI25 と他GPU（必要なら 3060 / 9070XT）で簡易比較条件を決める
- [ ] 速度比較用に同一プロンプト・同一 `num_predict` のベンチ手順を決める
...
- [ ] `libggml-hip.so` / `libggml-cpu*.so` の存在確認手順を `MI25_environment-setup.md` に昇格反映
- [x] backend 欠落時に「CPU fallback へ silently 移行」ではなく、即気づける運用メモを書く

## 3. 残務: ドキュメント整理
- [ ] `MI25_environment-setup.md` に「検証済み構成」を追記
- [ ] `MI25_environment-setup.md` に「backend 欠落時の症状」を追記
- [ ] `MI25_environment-setup.md` に「確認コマンド集」を整理して載せる
- [ ] `MI25_environment-setup-worklog.md` の節見出しを整えて読みやすくする
- [ ] 証跡ファイル名一覧を worklog 末尾にまとめる
- [ ] `ROCm-MI25-build` 側の README に今回の結論を反映する
- [ ] README の関連ファイル欄を、実在するファイルだけに限定して最終確認する

## 4. 残務: スクリプト整理
- [ ] `tinyllama-gpu-path-check.sh` のコメントを少し増やす
- [ ] 出力 TSV の列説明を README か docs に追記する
- [ ] `deepseek-r1-14b.sh` を汎用化する
- [ ] モデル名を引数に取る共通検証スクリプトへ寄せる
- [ ] `vega_path_check_logs/` のログ命名規則を簡単に定義する
- [ ] 生成ログ・journal・rocm-smi の3点セットを1ユニットとして扱う説明を書く
...
## 7. 低優先だけどやるとよさそう
- [ ] `tokens/sec` を簡易計算して記録する
- [ ] `tinyllama` と `deepseek-r1:14b` の VRAM 使用量の目安を書く
